...
class AddDda extends Component {
  ...
  handleAddDda = (event) => {
   ...
   /*if(dda_phone.trim().length<6)
   {
     alert("Enter 10 digit valid phone number");
   }
   const regex = /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i;
   if(! dda_email || regex.test( dda_email) === false){
    alert("Enter valid email");
   }*/
   /*var reg = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,32}$/;
   var test = reg.test( dda_password);
   if (test) {
       
   } else{
       alert('Enter a Strong Password length should be between 8-32 it must contain ateast one number , one lowercase alphabet and one uppercase alphabet ');
   }*/
    /*const state =
