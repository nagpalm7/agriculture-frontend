...
class LocationFilter extends Component {
  ...
  changeDistrict = (e) => {
    if (e) {
      const districtId = e.toString().split('_')[1];
      this.setState(
        {
          ...this.state,
      ...
      <>
        ...
        <Modal
          ...
          <Form
            ...
            <Form.Item label="Select DDA" name="dda">
              <Select showSearch placeholder="Select DDA">
                <Option value={undefined}>NO DDA</Option>
                {this.state.dda.length > 0 ? (
                  this.state.dda.map((dda) => {
                    return (
                      <Option
                        key={dda.id}
                        value={`${dda.user.username}_${dda.id}`}>
                        {dda.user.username}
                      </Option>
                    );
                  })
